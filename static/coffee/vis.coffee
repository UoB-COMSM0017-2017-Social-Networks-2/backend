...
Bubbles = () ->
  ...
  # ---
  click = (d) ->
    current_interval=decodeURIComponent(location.hash).split("==").pop()
    location.replace("#" + encodeURIComponent(idValue(d)+"=="+current_interval))
...
$ ->
  ...
  d3.select("#time")
    .on "input", () ->
      ...
      location.replace("#" + encodeURIComponent(current_topic+"=="+key))
